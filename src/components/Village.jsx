...
export default function Feed() {
  ...
  const handleComment = async (postId) => {
    ...
    if (!storedToken || !storedUserId || !commentText?.trim()) {
      return alert('Comment cannot be empty or unauthenticated');
    }
    ...
    return (
      <div key={post._id} className="bg-white shadow-md rounded-lg p-4 mb-6">
        <div className="flex items-center gap-3 mb-3">
          ...
          />
          <span className="font-semibold text-gray-800">{post?.userId?.username || 'Unknown User'}</span>
        </div>
            ...
            <div className="mt-4 space-y-3">
              {post.comments?.map((comment, i) => (
                <div key={i} className="flex items-start gap-3">
                  ...
                  />
                  <div>
                    <p className="font-semibold text-sm">{comment?.userId?.username || 'User'}</p>
